...
export default function Incharge_Dashboard() {
  ...
  const fetchArea = async () => {
    ...
    const response = await axios
      .post(
        `${process.env.REACT_APP_BASE_URL}/get-schedule-incharge`,
        {
      ...
      .then((e) => {
        ...
        const Parkingdata = e.data?.data?.data[0];
        console.log(e.data?.data?.data?.type_parking_space);
        console.log(e.data?.data?.data?.id);

        if (Parkingdata) {
          set_parkingAreaType(Parkingdata?.type_parking_space);
          set_parkingId(Parkingdata?.id);
        } else {
          return null;
        }
      })
    ...
    <div className="flex items-center justify-between h-screen w-screen">
      <div className="flex flex-1 flex-col justify-between bg-white h-screen ">
        ...
        <div className="relative w-full flex-col overflow-y-auto">
          ...
          <div className="flex flex-1 justify-center items-center">
            <div className="flex mt-5 w-fit justify-center items-center bg-white">
              <img
